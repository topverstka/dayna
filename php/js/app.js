...
playVideo()
function playVideo() {
    const iframeBody = document.querySelector('.video__iframe')

    iframeBody.addEventListener('click', () => {
        if (!iframeBody.classList.contains('_play')) {
            const data = iframeBody.dataset.youtubeSrc
            const iframe = iframeBody.querySelector('iframe')

            iframeBody.classList.add('_play')
            iframe.src = data
